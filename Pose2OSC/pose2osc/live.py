        ...
        try:
            if interactive:
                while True:
                    ...
                    selected = _select_hands(result, handedness)
                    if selected:
    ...
    with _open_capture(cv2, camera, width, height) as capture:
        ...
        try:
            while True:
                ...
                prediction = model.predict(model_landmarks, detected_handedness)
                state = tracker.update(prediction)
...
def _max_hands_for_mode(handedness: str | None) -> int:
    return 2 if _wants_both_hands(handedness) else 1


def _wants_both_hands(handedness: str | None) -> bool:
    return bool(handedness and handedness.lower() in {"both", "two", "2"})


def _missing_hand_message(handedness: str | None) -> str:
    if _wants_both_hands(handedness):
        return "Waiting for both hands"
...
def _select_hands(
    ...
    for index, hand_landmarks in enumerate(result.multi_hand_landmarks):
        ...
        if desired_handedness and desired_handedness.lower() != "any":
            if _wants_both_hands(desired_handedness):
                pass
            elif label.lower() != desired_handedness.lower():
                continue
        selected.append((
    ...
    if _wants_both_hands(desired_handedness):
        ...
        if "right" not in by_label or "left" not in by_label:
            return []
