...
to the manifest. Repeat for natural variations, then press `q` or `Esc` to
quit the capture window. Run one gesture label per capture session.

```bash
pose2osc enroll delay_hold --manifest manifests/theremin_set.json --hand Both --show
```

Each `space` press runs on the live MediaPipe hand-tracking feed, captures the
recent hand window, and writes those samples to the manifest immediately. For
theremin-style two-hand performance, use `--hand Both`; capture waits until
MediaPipe sees both hands and stores right-hand plus left-hand features
together. Aim for 5-10 captures per gesture.

To add another gesture, launch a new capture session:

```bash
pose2osc enroll filter_grab --manifest manifests/theremin_set.json --hand Both --show
```

To rebuild one label from scratch:

```bash
pose2osc enroll delay_hold --manifest manifests/theremin_set.json --hand Both --show --replace
```
...
```bash
pose2osc run --manifest manifests/theremin_set.json --hand Both --host 127.0.0.1 --port 8000 --split-axes --show
```
