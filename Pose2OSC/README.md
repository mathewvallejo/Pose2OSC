...
your home-folder venvs directory. This keeps MediaPipe, OpenCV, and NumPy away
from other project environments while keeping all runtime environments in one
place.
...
```bash
python3 -m venv ~/venvs/pose2osc_runtime_env
source ~/venvs/pose2osc_runtime_env/bin/activate
cd "/path/to/Pose2OSC"
python -m pip install --upgrade pip
python -m pip install -e ".[live]"
