...
class FeatureTests(unittest.TestCase):
    ...
    def test_two_hand_features_concatenate_right_then_left(self):
        config = FeatureConfig()
        right = open_hand()
        left = open_hand(offset=(0.5, 0.0, 0.0), scale=0.9)

        single_width = len(extract_features(right, config, "Right"))
        two_hand = extract_features(right + left, config, "Both")

        self.assertEqual(len(two_hand), single_width * 2)
